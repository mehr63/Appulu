...
namespace Appulu.Core.Caching
{
    ...
    public partial class RedisCacheManager : IStaticCacheManager
    {
        ...
        protected virtual async Task RemoveAsync(string key)
        {
            ...
            if (key.Equals(AppCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase))
                return;
        ...
        protected virtual async Task RemoveByPatternAsync(string pattern)
        {
            ...
            foreach (var endPoint in _connectionWrapper.GetEndPoints())
            {
                ...
                //we should always persist the data protection key list
                keys = keys.Where(key => !key.ToString().Equals(AppCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase));
        ...
        public virtual T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            if ((cacheTime ?? AppCachingDefaults.CacheTime) > 0)
                this.SetAsync(key, result, cacheTime ?? AppCachingDefaults.CacheTime).Wait();
