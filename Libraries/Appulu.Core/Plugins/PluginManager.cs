...
using Appulu.Core.ComponentModel;
...
namespace Appulu.Core.Plugins
{
    ...
    public class PluginManager
    {
        ...
        private static IEnumerable<KeyValuePair<string, PluginDescriptor>> GetDescriptionFilesAndDescriptors(string pluginFolder)
        {
            ...
            foreach (var descriptionFile in _fileProvider.GetFiles(pluginFolder, AppPluginDefaults.DescriptionFileName, false))
            {
            ...
            //it's required: https://www.Appulu.com/boards/t/17455/load-plugins-based-on-their-displayorder-on-startup.aspx
        ...
        private static IList<string> GetInstalledPluginNames(string filePath)
        {
            ...
            {
                //if not, try to parse the file that was used in previous appulu versions
                ...
                SaveInstalledPluginNames(pluginSystemNames, _fileProvider.MapPath(AppPluginDefaults.InstalledPluginsFilePath));
        ...
        private static Assembly PerformFileDeploy(string plug, ApplicationPartManager applicationPartManager, AppConfig config, string shadowCopyPath = "")
        {
            var parent = string.IsNullOrEmpty(plug) ? string.Empty : _fileProvider.GetParentDirectory(plug);

            if (string.IsNullOrEmpty(parent))
                throw new InvalidOperationException($"The plugin directory for the {_fileProvider.GetFileName(plug)} file exists in a folder outside of the allowed appulu folder hierarchy");
        ...
        /// <param name="plug">Plugin file info</param>
        /// <returns>Assembly</returns>
        private static Assembly RegisterPluginDefinition(AppConfig config, ApplicationPartManager applicationPartManager, string plug)
        {
        ...
        private static string ShadowCopyFile(string pluginFilePath, string shadowCopyPlugFolder)
        {
            ...
            if (_fileProvider.FileExists(shadowCopiedPlug))
            {
                ...
                else
                {
                    ...
                    //More info: https://www.Appulu.com/boards/t/11511/access-error-appplugindiscountrulesbillingcountrydll.aspx?p=4#60838
        ...
        /// <param name="applicationPartManager">Application part manager</param>
        /// <param name="config">Config</param>
        public static void Initialize(ApplicationPartManager applicationPartManager, AppConfig config)
        {
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                var pluginFolder = _fileProvider.MapPath(AppPluginDefaults.Path);
                _shadowCopyFolder = _fileProvider.MapPath(AppPluginDefaults.ShadowCopyPath);
                _reserveShadowCopyFolder = _fileProvider.Combine(_fileProvider.MapPath(AppPluginDefaults.ShadowCopyPath), $"{AppPluginDefaults.ReserveShadowCopyPathName}{DateTime.Now.ToFileTimeUtc()}");
                ...
                try
                {
                    var installedPluginSystemNames = GetInstalledPluginNames(_fileProvider.MapPath(AppPluginDefaults.InstalledPluginsFilePath));
                    ...
                    if (config.ClearPluginShadowDirectoryOnStartup)
                    {
                        ...
                        foreach (var directory in _fileProvider.GetDirectories(_shadowCopyFolder, AppPluginDefaults.ReserveShadowCopyPathNamePattern))
                        {
                            try
                            {
        ...
        public static PluginDescriptor GetPluginDescriptorFromText(string text)
        {
            ...
            //appulu 2.00 didn't have 'SupportedVersions' parameter, so let's set it to "2.00"
        ...
        public static void SavePluginDescriptor(PluginDescriptor pluginDescriptor)
        {
            ...
            var filePath = _fileProvider.Combine(_fileProvider.GetDirectoryName(pluginDescriptor.OriginalAssemblyFile), AppPluginDefaults.DescriptionFileName);
            if (!_fileProvider.FileExists(filePath))
        ...
        public static string GetLogoUrl(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoUrl = $"{webHelper.GetStoreLocation()}plugins/{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{AppPluginDefaults.LogoFileName}.{logoExtension}";
            return logoUrl;
        }
