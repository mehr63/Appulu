...
namespace Appulu.Services.Plugins
{
    ...
    public class UploadService : IUploadService
    {
        ...
        public UploadService(IAppFileProvider fileProvider,
                    ...
                    using (var unzippedEntryStream = entry.Open())
                    {
                        using (var reader = new StreamReader(unzippedEntryStream))
                        {
                            ...
                            if (isPluginDescriptor)
                            {
                                ...
                                if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(AppVersion.CurrentVersion))
                                    throw new Exception($"This plugin doesn't support the current version - {AppVersion.CurrentVersion}");
                            }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    //ensure that the current version of Appulu is supported
                    if (!item.SupportedVersions?.Contains(AppVersion.CurrentVersion) ?? true)
                        continue;
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(AppVersion.CurrentVersion))
                        continue;
        ...
        public class UploadedItem
        {
            ...
            /// <summary>
            /// Gets or sets supported versions of Appulu
            /// </summary>
