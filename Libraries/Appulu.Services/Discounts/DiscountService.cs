...
namespace Appulu.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
            ...
            RequirementGroupInteractionType groupInteractionType, User user, List<string> errors)
        {
            ...
            foreach (var requirement in requirements)
            {
                if (requirement.IsGroup)
                {
                    ...
                    result = GetValidationResult(requirement.ChildRequirements, interactionType, user, errors);
                }
                else
                {
                    ...
                    if (!_pluginFinder.AuthorizedForUser(requirementRulePlugin.PluginDescriptor, user))
                        continue;
            ...
            string couponCode = null, string discountName = null, bool showHidden = false)
        {
            ...
            var cacheKey = string.Format(AppDiscountDefaults.DiscountAllCacheKey,
        ...
        public virtual IList<int> GetAppliedCategoryIds(DiscountForCaching discount, User user)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));

            var discountId = discount.Id;
            var cacheKey = string.Format(AppDiscountDefaults.DiscountCategoryIdsModelCacheKey,
                discountId,
        ...
        public virtual IList<int> GetAppliedManufacturerIds(DiscountForCaching discount, User user)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));

            var discountId = discount.Id;
            var cacheKey = string.Format(AppDiscountDefaults.DiscountManufacturerIdsModelCacheKey,
                discountId,
                string.Join(",", user.GetUserRoleIds()),
                _storeContext.CurrentStore.Id);
            var result = _cacheManager.Get(cacheKey, () =>
        ...
        public virtual IList<IDiscountRequirementRule> LoadAllDiscountRequirementRules(User user = null)
        {
            return _pluginFinder.GetPlugins<IDiscountRequirementRule>(user: user).ToList();
        }
        ...
        public virtual DiscountValidationResult ValidateDiscount(Discount discount, User user)
        {
            ...
            return ValidateDiscount(this.MapDiscount(discount), user);
        }
        ...
        public virtual DiscountValidationResult ValidateDiscount(Discount discount, User user, string[] couponCodesToValidate)
        {
            ...
            return ValidateDiscount(this.MapDiscount(discount), user, couponCodesToValidate);
        }
        ...
        /// <param name="user">User</param>
        /// <returns>Discount validation result</returns>
        public virtual DiscountValidationResult ValidateDiscount(DiscountForCaching discount, User user)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));
        ...
        /// <param name="couponCodesToValidate">Coupon codes to validate</param>
        /// <returns>Discount validation result</returns>
        public virtual DiscountValidationResult ValidateDiscount(DiscountForCaching discount, User user, string[] couponCodesToValidate)
        {
            if (discount == null)
                throw new ArgumentNullException(nameof(discount));
            ...
            var key = string.Format(AppDiscountDefaults.DiscountRequirementModelCacheKey, discount.Id);
            ...
            result.IsValid = GetValidationResult(requirementsForCaching, topLevelGroup.InteractionType.Value, user, errors);
            ...
            int? userId = null, int? orderId = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (userId.HasValue && userId.Value > 0)
                discountUsageHistory = discountUsageHistory.Where(historyRecord => historyRecord.Order != null && historyRecord.Order.UserId == userId.Value);
