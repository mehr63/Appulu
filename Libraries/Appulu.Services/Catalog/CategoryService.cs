...
namespace Appulu.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        public virtual IList<Category> GetAllCategories(int storeId = 0, bool showHidden = false, bool loadCacheableCopy = true)
        {
            ...
            {
                //cacheable copy
                var key = string.Format(AppCatalogDefaults.CategoriesAllCacheKey,
                    storeId,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            if (_commonSettings.UseStoredProcedureForLoadingCategories)
            {
                ...
                //pass allowed user role identifiers as comma-delimited string
                var userRoleIdsParameter = _dataProvider.GetStringParameter("UserRoleIds", !_catalogSettings.IgnoreAcl ? string.Join(",", _workContext.CurrentUser.GetUserRoleIds()) : string.Empty);
                ...
                var categories = _dbContext.EntityFromSql<Category>("CategoryLoadAllPaged",
                    showHiddenParameter, nameParameter, storeIdParameter, userRoleIdsParameter,
        ...
        {
            var key = string.Format(AppCatalogDefaults.CategoriesByParentCategoryIdCacheKey, parentCategoryId, showHidden, _workContext.CurrentUser.Id, _storeContext.CurrentStore.Id);
        ...
        {
            var cacheKey = string.Format(AppCatalogDefaults.CategoriesChildIdentifiersCacheKey,
                parentCategoryId,
                string.Join(",", _workContext.CurrentUser.GetUserRoleIds()),
        ...
        public virtual Category GetCategoryById(int categoryId)
        {
            ...
            var key = string.Format(AppCatalogDefaults.CategoriesByIdCacheKey, categoryId);
        ...
        public virtual void InsertCategory(Category category)
        {
            ...
            //cache
            _cacheManager.RemoveByPattern(AppCatalogDefaults.CategoriesPatternCacheKey);
            _staticCacheManager.RemoveByPattern(AppCatalogDefaults.CategoriesPatternCacheKey);
            _cacheManager.RemoveByPattern(AppCatalogDefaults.ProductCategoriesPatternCacheKey);
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            var key = string.Format(AppCatalogDefaults.ProductCategoriesAllByCategoryIdCacheKey, showHidden, categoryId, pageIndex, pageSize, _workContext.CurrentUser.Id, _storeContext.CurrentStore.Id);
            return _cacheManager.Get(key, () =>
            {
        ...
        public virtual IList<ProductCategory> GetProductCategoriesByProductId(int productId, int storeId, bool showHidden = false)
        {
            ...
            var key = string.Format(AppCatalogDefaults.ProductCategoriesAllByProductIdCacheKey, showHidden, productId, _workContext.CurrentUser.Id, storeId);
            return _cacheManager.Get(key, () =>
            {
                var query = from pc in _productCategoryRepository.Table
                            join c in _categoryRepository.Table on pc.CategoryId equals c.Id
