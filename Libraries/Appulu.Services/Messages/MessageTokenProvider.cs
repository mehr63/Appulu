...
namespace Appulu.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            {
                var createdOn = _dateTimeHelper.ConvertToUserTime(order.CreatedOnUtc, TimeZoneInfo.Utc, _dateTimeHelper.GetUserTimeZone(order.User));
        ...
        public virtual void AddUserTokens(IList<Token> tokens, User user)
        {
            ...
            tokens.Add(new Token("User.FirstName", _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.FirstNameAttribute)));
            tokens.Add(new Token("User.LastName", _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.LastNameAttribute)));
            tokens.Add(new Token("User.VatNumber", _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.VatNumberAttribute)));
            tokens.Add(new Token("User.VatNumberStatus", ((VatNumberStatus)_genericAttributeService.GetAttribute<int>(user, AppUserDefaults.VatNumberStatusIdAttribute)).ToString()));

            var customAttributesXml = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.CustomUserAttributes);
            ...
            var passwordRecoveryUrl = RouteUrl(routeName: "PasswordRecoveryConfirm", routeValues: new { token = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.PasswordRecoveryTokenAttribute), email = user.Email });
            var accountActivationUrl = RouteUrl(routeName: "AccountActivation", routeValues: new { token = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.AccountActivationTokenAttribute), email = user.Email });
            var emailRevalidationUrl = RouteUrl(routeName: "EmailRevalidation", routeValues: new { token = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.EmailRevalidationTokenAttribute), email = user.Email });
        ...
        public virtual void AddVendorTokens(IList<Token> tokens, Vendor vendor)
        {
            ...
            var vendorAttributesXml = _genericAttributeService.GetAttribute<string>(vendor, AppVendorDefaults.VendorAttributes);
        ...
        {
            var forumService = EngineContext.Current.Resolve<IForumService>();
            tokens.Add(new Token("Forums.PostAuthor", _userService.FormatUserName(forumPost.User)));
