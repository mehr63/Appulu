...
namespace Appulu.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        protected virtual decimal GetShippingDiscount(User user, decimal shippingTotal, out List<DiscountForCaching> appliedDiscounts)
        {
            appliedDiscounts = new List<DiscountForCaching>();
        ...
        /// <param name="appliedDiscounts">Applied discounts</param>
        /// <returns>Order discount</returns>
        protected virtual decimal GetOrderTotalDiscount(User user, decimal orderTotal, out List<DiscountForCaching> appliedDiscounts)
        {
            appliedDiscounts = new List<DiscountForCaching>();
            var discountAmount = decimal.Zero;
            if (_catalogSettings.IgnoreDiscounts)
        ...
        protected virtual void UpdateTotal(UpdateOrderParameters updateOrderParameters, decimal subTotalExclTax,
            decimal discountAmountExclTax, decimal shippingTotalExclTax, Order updatedOrder, decimal taxTotal, User user)
        {
        ...
        protected virtual decimal UpdateShipping(UpdateOrderParameters updateOrderParameters, IList<ShoppingCartItem> restoredCart,
            decimal subTotalInclTax, decimal subTotalExclTax, Order updatedOrder, User user, out decimal shippingTotalInclTax, out decimal shippingTaxRate)
        {
                ...
                if (!IsFreeShipping(restoredCart, _shippingSettings.FreeShippingOverXIncludingTax ? subTotalInclTax : subTotalExclTax))
                {
                    ...
                    if (!string.IsNullOrEmpty(updatedOrder.ShippingRateComputationMethodSystemName))
                    {
                        ...
                        if (updatedOrder.PickUpInStore)
                        {
                            ...
                            {
                                var pickupPointsResponse = _shippingService.GetPickupPoints(updatedOrder.BillingAddress, updatedOrder.User,
                        ...
                        else
                        {
                            ...
                            var shippingOptionsResponse = _shippingService.GetShippingOptions(restoredCart, updatedOrder.ShippingAddress, updatedOrder.User, updatedOrder.ShippingRateComputationMethodSystemName, _storeContext.CurrentStore.Id);
                    ...
                    else
                    {
                        ...
                        if (updateOrderParameters.PickupPoint == null)
                        {
                            ...
                            var shippingRateComputationMethods =
                                _shippingService.LoadActiveShippingRateComputationMethods(_workContext.CurrentUser, _storeContext.CurrentStore.Id);
                            if (shippingRateComputationMethods.Any())
                            {
                                var shippingOptionsResponse = _shippingService.GetShippingOptions(restoredCart, user.ShippingAddress, _workContext.CurrentUser, storeId: _storeContext.CurrentStore.Id);
                                if (shippingOptionsResponse.Success)
                                {
                                    ...
                                    updatedOrder.ShippingAddress = (Address)user.ShippingAddress.Clone();
                                    shippingTotal = shippingOption.Rate;
                                }
                                else
                                    updateOrderParameters.Warnings.AddRange(shippingOptionsResponse.Errors);
        ...
        protected virtual decimal UpdateSubTotal(UpdateOrderParameters updateOrderParameters, IList<ShoppingCartItem> restoredCart,
            OrderItem updatedOrderItem, Order updatedOrder, User user,
            ...
            bool? useRewardPoints, User user, decimal orderTotal)
        {
            ...
            if (!useRewardPoints.HasValue)
                useRewardPoints = _genericAttributeService.GetAttribute<bool>(user, AppUserDefaults.UseRewardPointsDuringCheckoutAttribute, _storeContext.CurrentStore.Id);
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            //get the user 
            var user = cart.FirstOrDefault(item => item.User != null)?.User;
            ...
            foreach (var shoppingCartItem in cart)
            {
                ...
                var sciExclTax = _taxService.GetProductPrice(shoppingCartItem.Product, sciSubTotal, false, user, out var taxRate);
                var sciInclTax = _taxService.GetProductPrice(shoppingCartItem.Product, sciSubTotal, true, user, out taxRate);
            ...
            if (user != null)
            {
                var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
                var attributeValues = _checkoutAttributeParser.ParseCheckoutAttributeValues(checkoutAttributesXml);
                if (attributeValues != null)
                {
                    foreach (var attributeValue in attributeValues)
                    {
                        var caExclTax = _taxService.GetCheckoutAttributePrice(attributeValue, false, user, out var taxRate);
                        var caInclTax = _taxService.GetCheckoutAttributePrice(attributeValue, true, user, out taxRate);
        ...
        public virtual void UpdateOrderTotals(UpdateOrderParameters updateOrderParameters, IList<ShoppingCartItem> restoredCart)
        {
            ...
            //get the user 
            var user = restoredCart.FirstOrDefault(item => item.User != null)?.User;

            //sub total
            var subTotalExclTax = UpdateSubTotal(updateOrderParameters, restoredCart, updatedOrderItem, updatedOrder, user, out var subTotalInclTax, out var subTotalTaxRates, out var discountAmountExclTax);

            //shipping
            var shippingTotalExclTax = UpdateShipping(updateOrderParameters, restoredCart, subTotalInclTax, subTotalExclTax, updatedOrder, user, out var shippingTotalInclTax, out var shippingTaxRate);

            //tax rates
            var taxTotal = UpdateTaxRates(subTotalTaxRates, shippingTotalInclTax, shippingTotalExclTax, shippingTaxRate, updatedOrder);

            //total
            UpdateTotal(updateOrderParameters, subTotalExclTax, discountAmountExclTax, shippingTotalExclTax, updatedOrder, taxTotal, user);
        }
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart, decimal? subTotal = null)
        {
            ...
            var user = cart.FirstOrDefault(item => item.User != null)?.User;
            if (user != null && user.UserRoles.Where(role => role.Active).Any(role => role.FreeShipping))
                return true;
            ...
            IList<ShoppingCartItem> cart, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            //discount
            var discountAmount = GetShippingDiscount(cart.FirstOrDefault(item => item.User != null)?.User, adjustedRate, out appliedDiscounts);
            ...
            out decimal taxRate, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            if (user != null)
                shippingOption = _genericAttributeService.GetAttribute<ShippingOption>(user, AppUserDefaults.SelectedShippingOptionAttribute, _storeContext.CurrentStore.Id);
            ...
            else
            {
                ...
                var shippingRateComputationMethods = _shippingService.LoadActiveShippingRateComputationMethods(_workContext.CurrentUser, _storeContext.CurrentStore.Id);
                if (!shippingRateComputationMethods.Any() && !_shippingSettings.AllowPickUpInStore)
                    throw new AppException("Shipping rate computation method could not be loaded");
            ...
            out SortedDictionary<decimal, decimal> taxRates, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            if (user != null)
            {
                paymentMethodSystemName = _genericAttributeService.GetAttribute<string>(user,
                    AppUserDefaults.SelectedPaymentMethodAttribute, _storeContext.CurrentStore.Id);
            }
