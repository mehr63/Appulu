...
namespace Appulu.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            bool ensureOnlyActiveCheckoutAttributes = false)
        {
            ...
            {
                _userService.ResetCheckoutData(shoppingCartItem.User, shoppingCartItem.StoreId);
            }
                ...
                shoppingCartItem.ShoppingCartType == ShoppingCartType.ShoppingCart)
            {
                ...
                var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(user, AppUserDefaults.CheckoutAttributes, storeId);
                checkoutAttributesXml = _checkoutAttributeParser.EnsureOnlyActiveAttributes(checkoutAttributesXml, cart);
                _genericAttributeService.SaveAttribute(user, AppUserDefaults.CheckoutAttributes, checkoutAttributesXml, storeId);
            }
            ...
            var requiredShoppingCartItems = user.ShoppingCartItems
            ...
            DateTime? rentalEndDate = null)
        {
            ...
            foreach (var sci in shoppingCart.Where(a => a.ShoppingCartType == shoppingCartType))
            {
                ...
                if (sci.Product.UserEntersPrice)
                    //TODO should we use RoundingHelper.RoundPrice here?
                    userEnteredPricesEqual = Math.Round(sci.UserEnteredPrice, 2) == Math.Round(userEnteredPrice, 2);
                ...
                //found?
                if (attributesEqual && giftCardInfoSame && userEnteredPricesEqual && rentalInfoEqual)
                    return sci;
            }
        ...
        public virtual void MigrateShoppingCart(User fromUser, User toUser, bool includeCouponCodes)
        {
            ...
            {
                var sci = fromCart[i];
                AddToCart(toUser, sci.Product, sci.ShoppingCartType, sci.StoreId,
            ...
            var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(fromUser, AppUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            _genericAttributeService.SaveAttribute(toUser, AppUserDefaults.CheckoutAttributes, checkoutAttributesXml, _storeContext.CurrentStore.Id);
        }
            ...
            out int cycleLength, out RecurringProductCyclePeriod cyclePeriod, out int totalCycles)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    throw new AppException($"Product (Id={sci.ProductId}) cannot be loaded");
                }
