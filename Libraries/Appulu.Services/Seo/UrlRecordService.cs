...
namespace Appulu.Services.Seo
{
    ...
    public partial class UrlRecordService : IUrlRecordService
    {
        ...
        {
            //cache
            return _cacheManager.Get(AppSeoDefaults.UrlRecordAllCacheKey, () =>
            {
        ...
        public virtual void DeleteUrlRecord(UrlRecord urlRecord)
        {
            ...
            //cache
            _cacheManager.RemoveByPattern(AppSeoDefaults.UrlRecordPatternCacheKey);
        }
        ...
        public virtual UrlRecordForCaching GetBySlugCached(string slug)
        {
            ...
            //gradual loading
            var key = string.Format(AppSeoDefaults.UrlRecordBySlugCacheKey, slug);
        ...
        public virtual string GetActiveSlug(int entityId, string entityName, int languageId)
        {
            if (_localizationSettings.LoadAllUrlRecordsOnStartup)
            {
                var key = string.Format(AppSeoDefaults.UrlRecordActiveByIdNameLanguageCacheKey, entityId, entityName, languageId);
                return _cacheManager.Get(key, () =>
                {
                    //load all records (we know they are cached)
                    var source = GetAllUrlRecordsCached();
