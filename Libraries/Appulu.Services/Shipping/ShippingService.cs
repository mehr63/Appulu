...
using Appulu.Services.Shipping.Pickup;
...
namespace Appulu.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        public virtual IList<IShippingRateComputationMethod> LoadActiveShippingRateComputationMethods(User user = null, int storeId = 0)
        {
            return LoadAllShippingRateComputationMethods(user, storeId)
        ...
        /// <param name="storeId">Load records allowed only in a specified store; pass 0 to load all records</param>
        /// <returns>Shipping rate computation methods</returns>
        public virtual IList<IShippingRateComputationMethod> LoadAllShippingRateComputationMethods(User user = null, int storeId = 0)
        {
            return _pluginFinder.GetPlugins<IShippingRateComputationMethod>(user: user, storeId: storeId).ToList();
        }
        ...
        public virtual void DeleteShippingMethod(ShippingMethod shippingMethod)
        {
            ...
            _cacheManager.RemoveByPattern(AppShippingDefaults.ShippingMethodsPatternCacheKey);
        ...
        {
            var key = string.Format(AppShippingDefaults.ShippingMethodsAllCacheKey, filterByCountryId ?? 0);
        ...
        public virtual void DeleteWarehouse(Warehouse warehouse)
        {
            ...
            //clear cache
            _cacheManager.RemoveByPattern(AppShippingDefaults.WarehousesPatternCacheKey);
        ...
        public virtual Warehouse GetWarehouseById(int warehouseId)
        {
            ...
            var key = string.Format(AppShippingDefaults.WarehousesByIdCacheKey, warehouseId);
        ...
        public virtual IList<IPickupPointProvider> LoadActivePickupPointProviders(User user = null, int storeId = 0)
        {
            return LoadAllPickupPointProviders(user, storeId).Where(provider => _shippingSettings.ActivePickupPointProviderSystemNames
                .Contains(provider.PluginDescriptor.SystemName, StringComparer.InvariantCultureIgnoreCase)).ToList();
        }
        ...
        /// <param name="storeId">Load records allowed only in a specified store; pass 0 to load all records</param>
        /// <returns>Pickup point providers</returns>
        public virtual IList<IPickupPointProvider> LoadAllPickupPointProviders(User user = null, int storeId = 0)
        {
            return _pluginFinder.GetPlugins<IPickupPointProvider>(user: user, storeId: storeId).ToList();
        }
            ...
            bool includeCheckoutAttributes = true, bool ignoreFreeShippedItems = false)
        {
            ...
            if (request.User == null || !includeCheckoutAttributes) 
                return totalWeight;
            var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(request.User, AppUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            ...
            int storeId = 0)
        {
            ...
            var shippingRateComputationMethods = LoadActiveShippingRateComputationMethods(user, storeId);
        ...
        public virtual GetPickupPointsResponse GetPickupPoints(Address address, User user = null, string providerSystemName = null, int storeId = 0)
        {
            var result = new GetPickupPointsResponse();
            var pickupPointsProviders = LoadActivePickupPointProviders(user, storeId);
