...
using Appulu.Plugin.Payments.Worldpay.Domain.Responses;
    ...
    public class WorldpayPaymentManager
    {
            ...
            where TRequest : WorldpayPaymentRequest where TResponse : WorldpayPaymentResponse
        {
            ...
            var response = ProcessRequest<TRequest, TResponse>(request)
                ?? throw new AppException("An error occurred. Details in the log.");

            //whether request is approved
            if (response.Code != ResponseCode.Approved)
                throw new AppException($"The request was {response.Code} for a reason '{response.Result}'. {response.Message}");
            ...
            where TRequest : WorldpayRequest where TResponse : WorldpayResponse
        {
            ...
            catch (Exception exception)
            {
                ...
                {
                    //log errors
                    _logger.Error(errorMessage, exception, _workContext.CurrentUser);
                }
        ...
        /// <summary>
        /// Get a user from the Vault
        /// </summary>
        /// <param name="userId">User identifier</param>
        /// <returns>Vault user</returns>
        public VaultUser GetUser(string userId)
        {
            ...
            var response = ProcessRequest<GetUserRequest, GetUserResponse>(request);
            if (response != null && response.Code == ResponseCode.Approved)
            {
                ...
                if (!response.UserId.Equals("error", StringComparison.InvariantCultureIgnoreCase))
                    return response.User;

                //log this strange error
                _logger.Error($"Failed to retrieve user. {response.User.Notes}", user: _workContext.CurrentUser);
            }
            else
            {
                //log errors
                _logger.Error($"The request was {response?.Code} for a reason '{response?.Result}'. {response?.Message}", user: _workContext.CurrentUser);
            }
        ...
        /// <summary>
        /// Create a user in the Vault
        /// </summary>
        /// <param name="request">Request parameters to create a user</param>
        /// <returns>Vault user</returns>
        public VaultUser CreateUser(CreateUserRequest request)
        {
            //set developer credentials
            SetDeveloperCredentials(request);

            //try to create a user
            var response = ProcessRequest<CreateUserRequest, CreateUserResponse>(request);
            if (response != null && response.Code == ResponseCode.Approved)
                return response.User;
        ...
        /// <summary>
        /// Update a user in the Vault
        /// </summary>
        /// <param name="request">Request parameters to update a user</param>
        /// <returns>Vault user</returns>
        public VaultUser UpdateUser(UpdateUserRequest request)
        {
            //set developer credentials
            SetDeveloperCredentials(request);

            //try to update a user
            var response = ProcessRequest<UpdateUserRequest, UpdateUserResponse>(request);
            if (response != null && response.Code == ResponseCode.Approved)
                return response.User;
        ...
        /// <summary>
        /// Delete a user credit card from the Vault
        /// </summary>
