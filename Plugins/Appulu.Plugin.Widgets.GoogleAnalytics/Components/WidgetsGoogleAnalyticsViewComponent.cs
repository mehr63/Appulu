...
namespace Appulu.Plugin.Widgets.GoogleAnalytics.Components
{
    [ViewComponent(Name = "WidgetsGoogleAnalytics")]
    public class WidgetsGoogleAnalyticsViewComponent : AppViewComponent
    {
        #region Fields
        ...
        private string GetEcommerceScript(Order order)
        {
            ...
            if (_googleAnalyticsSettings.IncludeUserId && !_workContext.CurrentUser.IsGuest())
                userIdCode = $"gtag('set', {{'user_id': '{_workContext.CurrentUser.Id}'}});{Environment.NewLine}";
            analyticsTrackingScript = analyticsTrackingScript.Replace("{USER_TRACKING}", userIdCode);
