...
namespace Appulu.Plugin.Payments.Square.Services
{
    ...
    public class SquarePaymentManager
    {
        ...
        private Configuration CreateApiConfiguration()
        {
            ...
            {
                throw new AppException($"Sandbox access token should start with '{SquarePaymentDefaults.SandboxCredentialsPrefix}'");
            }
        ...
        public IList<Location> GetActiveLocations()
        {
            try
            {
                ...
                if (listLocationsResponse == null)
                    throw new AppException("No service response");
                ...
                if (listLocationsResponse.Errors?.Any() ?? false)
                {
                    ...
                    throw new AppException($"There are errors in the service response. {errorsMessage}");
                }
                ...
                if (!activeLocations?.Any() ?? true)
                    throw new AppException("There are no active locations for the account");
            ...
            {
                //log full error
                _logger.Error($"Square payment error: {exception.Message}.", exception, _workContext.CurrentUser);
        ...
        public User GetUser(string userId)
        {
            try
            {
                //whether passed user identifier exists
                ...
                //get user by identifier
                var retrieveUserResponse = usersApi.RetrieveUser(userId);
                if (retrieveUserResponse == null)
                    throw new AppException("No service response");

                //check whether there are errors in the service response
                if (retrieveUserResponse.Errors?.Any() ?? false)
                {
                    var errorsMessage = string.Join(";", retrieveUserResponse.Errors.Select(error => error.ToString()));
                    throw new AppException($"There are errors in the service response. {errorsMessage}");
                }
        ...
        /// Create the new user
        /// </summary>
        /// <param name="userRequest">Request parameters to create user</param>
        /// <returns>User</returns>
        public User CreateUser(CreateUserRequest userRequest)
        {
            try
            {
                ...
                //create the new user
                var createUserResponse = usersApi.CreateUser(userRequest);
                if (createUserResponse == null)
                    throw new AppException("No service response");

                //check whether there are errors in the service response
                if (createUserResponse.Errors?.Any() ?? false)
                {
                    var errorsMessage = string.Join(";", createUserResponse.Errors.Select(error => error.ToString()));
                    throw new AppException($"There are errors in the service response. {errorsMessage}");
                }
        ...
        /// <summary>
        /// Create the new card of the user
        /// </summary>
        /// <param name="userId">User ID</param>
        /// <param name="cardRequest">Request parameters to create card of the user</param>
        /// <returns>Card</returns>
        public Card CreateUserCard(string userId, CreateUserCardRequest cardRequest)
        {
            try
            {
                //create user API
                ...
                //create the new card of the user
                var createUserCardResponse = usersApi.CreateUserCard(userId, cardRequest);
                if (createUserCardResponse == null)
                    throw new AppException("No service response");

                //check whether there are errors in the service response
                if (createUserCardResponse.Errors?.Any() ?? false)
                {
                    var errorsMessage = string.Join(";", createUserCardResponse.Errors.Select(error => error.ToString()));
                    throw new AppException($"There are errors in the service response. {errorsMessage}");
                }

                return createUserCardResponse.Card;
            }
            catch (Exception exception)
            {
                //log full error
        ...
        public (Transaction, string) GetTransaction(string transactionId)
        {
            try
            {
                ...
                if (selectedLocation == null)
                    throw new AppException("Location is a required parameter for payment requests");
            ...
            catch (Exception exception)
            {
                ...
                _logger.Error($"Square payment error: {errorMessage}.", exception, _workContext.CurrentUser);
        ...
        public string GenerateAuthorizeUrl(string verificationString)
        {
            ...
            var permissionScopes = new List<string>
            {
                ...
                //GET endpoints related to user management.
                "USERS_READ",

                //POST, PUT, and DELETE endpoints related to user management.
                "USERS_WRITE",
