    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        var priceAdjustment = _taxService.GetProductPrice(product,
                            _priceCalculationService.GetProductAttributeValuePriceAdjustment(attributeValue, order.User), out _);
            ...
            var model = new OrderListModel
            {
                ...
                Data = orders.Select(order =>
                {
                    ...
                    var orderModel = new OrderModel
                    {
                        ...
                        UserFullName = $"{order.BillingAddress.FirstName} {order.BillingAddress.LastName}",
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            if (order != null)
            {
                ...
                model.UserInfo = order.User.IsRegistered() ? order.User.Email : _localizationService.GetResource("Admin.Users.Guest");
        ...
        public virtual AddProductToOrderModel PrepareAddProductToOrderModel(AddProductToOrderModel model, Order order, Product product)
        {
            ...
            var presetQty = 1;
            var presetPrice = _priceCalculationService.GetFinalPrice(product, order.User, decimal.Zero, true, presetQty);
            var presetPriceInclTax = _taxService.GetProductPrice(product, presetPrice, true, order.User, out _);
            var presetPriceExclTax = _taxService.GetProductPrice(product, presetPrice, false, order.User, out _);
