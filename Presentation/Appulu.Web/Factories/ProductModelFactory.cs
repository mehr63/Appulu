...
namespace Appulu.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual void PrepareSimpleProductOverviewPriceModel(Product product, ProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    var minPossiblePriceWithDiscount = _priceCalculationService.GetFinalPrice(product, _workContext.CurrentUser, includeDiscounts: true);
                    ...
                    if (product.HasTierPrices)
                    {
                        tierPrices.AddRange(product.TierPrices.OrderBy(tp => tp.Quantity)
                            ...
                            .FilterForUser(_workContext.CurrentUser)
                            .FilterByDate()
            ...
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var finalPriceWithDiscountBase = _taxService.GetProductPrice(product, _priceCalculationService.GetFinalPrice(product, _workContext.CurrentUser, includeDiscounts: true), out _);
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                        {
                            var attributeValuePriceAdjustment = _priceCalculationService.GetProductAttributeValuePriceAdjustment(attributeValue, updatecartitem?.User ?? _workContext.CurrentUser);
        ...
        protected virtual IList<ProductDetailsModel.TierPriceModel> PrepareProductTierPriceModels(Product product)
        {
            ...
            var model = product.TierPrices.OrderBy(x => x.Quantity)
                   ...
                   .Select(tierPrice =>
                   {
                       ...
                       _workContext.CurrentUser, decimal.Zero, _catalogSettings.DisplayTierPricesWithDiscounts, tierPrice.Quantity), out decimal _);
        ...
        public virtual ProductReviewsModel PrepareProductReviewsModel(ProductReviewsModel model, Product product)
        {
            ...
            foreach (var pr in productReviews)
            {
                ...
                foreach (var q in _reviewTypeService.GetProductReviewReviewTypeMappingsByProductReviewId(pr.Id))
                {
                    productReviewModel.AdditionalProductReviewList.Add(new ProductReviewReviewTypeMappingModel
                    {
                        ...
                        VisibleToAllUsers = q.ReviewType.VisibleToAllUsers || _workContext.CurrentUser.Id == pr.UserId,
                    });
                }
            ...
            model.AddProductReview.CanCurrentUserLeaveReview = _catalogSettings.AllowAnonymousUsersToReviewProduct || !_workContext.CurrentUser.IsGuest();
