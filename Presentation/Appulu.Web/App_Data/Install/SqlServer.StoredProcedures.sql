CREATE FUNCTION [dbo].[app_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[app_getnotnullnotempty]
(
...
CREATE FUNCTION [dbo].[app_getprimarykey_indexname]
(
...
CREATE FUNCTION [dbo].[app_padright]
(
    @source INT, 
    @symbol NVARCHAR(MAX), 
    @length INT
...
BEGIN
	...
	BEGIN
		...
		UNION
		SELECT lp.EntityId
		FROM Localization.LocalizedProperty lp with (NOLOCK)
		WHERE
		...
		BEGIN
			...
			FROM [Catalog].Product_ProductTag_Mapping pptm with(NOLOCK) INNER JOIN [Catalog].ProductTag pt with(NOLOCK) ON pt.Id = pptm.ProductTag_Id
			...
			UNION
			SELECT pptm.Product_Id
			FROM Localization.LocalizedProperty lp with (NOLOCK) INNER JOIN [Catalog].Product_ProductTag_Mapping pptm with(NOLOCK) ON lp.EntityId = pptm.ProductTag_Id
			WHERE
	...
	SELECT CAST(data as int) FROM [app_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [app_splitstring_to_table](@AllowedUserRoleIds, ',')
	...
	SELECT p.Id
	FROM
		[Catalog].Product p with (NOLOCK)'
	
	IF @CategoryIdsCount > 0
	BEGIN
		SET @sql = @sql + '
		INNER JOIN [Catalog].Product_Category_Mapping pcm with (NOLOCK)
	...
	BEGIN
		SET @sql = @sql + '
		INNER JOIN [Catalog].Product_Manufacturer_Mapping pmm with (NOLOCK)
	...
	BEGIN
		SET @sql = @sql + '
		INNER JOIN [Catalog].Product_ProductTag_Mapping pptm with (NOLOCK)
	...
	BEGIN
		...
		AND  
			(
				...
				OR
				(p.UseMultipleWarehouses > 0 AND
					EXISTS (SELECT 1 FROM [Catalog].ProductWarehouseInventory [pwi]
		...
		AND (p.SubjectToAcl = 0 OR EXISTS (
			...
			WHERE
				[fcr].UserRoleId IN (
					...
					FROM Security.[AclRecord] acl with (NOLOCK)
	...
	BEGIN
		...
		AND (p.LimitedToStores = 0 OR EXISTS (
			SELECT 1 FROM Stores.[StoreMapping] sm with (NOLOCK)
	...
	BEGIN		
        ...
        SET @sql_filterableSpecs = '
	        ...
	        FROM [Catalog].[Product_SpecificationAttribute_Mapping] [psam] WITH (NOLOCK)
	...
	SELECT CAST(data as int) FROM [app_splitstring_to_table](@FilteredSpecs, ',') 
    ...
    FROM #FilteredSpecs fs INNER JOIN [Catalog].SpecificationAttributeOption sao ON sao.Id = fs.SpecificationAttributeOptionId
	...
	FROM
		#PageIndex [pi]
		INNER JOIN [Catalog].Product p with (NOLOCK) on p.Id = [pi].[ProductId]
	WHERE
...
BEGIN
	...
	LEFT JOIN [Catalog].Product_ProductTag_Mapping pptm with (NOLOCK) ON pt.[Id] = pptm.[ProductTag_Id]
	LEFT JOIN [Catalog].Product p with (NOLOCK) ON pptm.[Product_Id] = p.[Id]
	WHERE
		p.[Deleted] = 0
		AND p.Published = 1
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''appuluFullTextCatalog'')
		CREATE FULLTEXT CATALOG [appuluFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[app_getprimarykey_indexname] ('Product') +  '] ON [appuluFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[app_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [appuluFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[app_getprimarykey_indexname] ('ProductTag') +  '] ON [appuluFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''appuluFullTextCatalog'')
		DROP FULLTEXT CATALOG [appuluFullTextCatalog]
	')
END
GO
...
AS
BEGIN
	IF EXISTS(SELECT * FROM Localization.[Language] WHERE [Id] = @LanguageId)
	BEGIN
		...
		BEGIN
			IF (EXISTS (SELECT 1 FROM Localization.[LocaleStringResource] WHERE LanguageId=@LanguageId AND ResourceName=@ResourceName))
			BEGIN
				IF (@UpdateExistingResources = 1)
				BEGIN
					UPDATE Localization.[LocaleStringResource]
			...
			ELSE 
			BEGIN
				INSERT INTO Localization.[LocaleStringResource]
				(
					[LanguageId],
					[ResourceName],
					[ResourceValue]
...
BEGIN
	...
	SELECT [Id] FROM Users.[User] c with (NOLOCK)
	WHERE
	--created from
	...
	AND
	--shopping cart items
	((@OnlyWithoutShoppingCart=0) OR (NOT EXISTS(SELECT 1 FROM Orders.[ShoppingCartItem] sci with (NOLOCK) inner join Users.[User] with (NOLOCK) on sci.[UserId]=c.[Id])))
	AND
	--guests only
	(EXISTS(SELECT 1 FROM Users.[User_UserRole_Mapping] ccrm with (NOLOCK) inner join Users.[User] with (NOLOCK) on ccrm.[User_Id]=c.[Id] inner join Users.[UserRole] cr with (NOLOCK) on cr.[Id]=ccrm.[UserRole_Id] WHERE cr.[SystemName] = N'Guests'))
	AND
	--no orders
	(NOT EXISTS(SELECT 1 FROM Orders.[Order] o with (NOLOCK) inner join Users.[User] with (NOLOCK) on o.[UserId]=c.[Id]))
	AND
	--no blog comments
	(NOT EXISTS(SELECT 1 FROM Blogs.[BlogComment] bc with (NOLOCK) inner join Users.[User] with (NOLOCK) on bc.[UserId]=c.[Id]))
	AND
	--no news comments
	(NOT EXISTS(SELECT 1 FROM News.[NewsComment] nc  with (NOLOCK)inner join Users.[User] with (NOLOCK) on nc.[UserId]=c.[Id]))
	AND
	--no product reviews
	(NOT EXISTS(SELECT 1 FROM [Catalog].[ProductReview] pr with (NOLOCK) inner join Users.[User] with (NOLOCK) on pr.[UserId]=c.[Id]))
	AND
	--no product reviews helpfulness
	(NOT EXISTS(SELECT 1 FROM [Catalog].[ProductReviewHelpfulness] prh with (NOLOCK) inner join Users.[User] with (NOLOCK) on prh.[UserId]=c.[Id]))
	AND
	--no poll voting
	(NOT EXISTS(SELECT 1 FROM Polls.[PollVotingRecord] pvr with (NOLOCK) inner join Users.[User] with (NOLOCK) on pvr.[UserId]=c.[Id]))
	AND
	--no forum topics 
	(NOT EXISTS(SELECT 1 FROM Forums.[ForumTopic] ft with (NOLOCK) inner join Users.[User] with (NOLOCK) on ft.[UserId]=c.[Id]))
	AND
	--no forum posts 
	(NOT EXISTS(SELECT 1 FROM Forums.[ForumPost] fp with (NOLOCK) inner join Users.[User] with (NOLOCK) on fp.[UserId]=c.[Id]))
	AND
	--no system accounts
	(c.IsSystemAccount = 0)
	...
	--delete attributes
	DELETE Common.[GenericAttribute]
...
BEGIN
	...
	INSERT INTO #FilteredUserRoleIds (UserRoleId)
	SELECT CAST(data AS INT) FROM [app_splitstring_to_table](@UserRoleIds, ',')
    ...
    DECLARE @lengthId INT = (SELECT LEN(MAX(Id)) FROM [Catalog].[Category])
    DECLARE @lengthOrder INT = (SELECT LEN(MAX(DisplayOrder)) FROM [Catalog].[Category])

    --get category tree
    ;WITH [CategoryTree]
    AS (SELECT [Category].[Id] AS [Id], dbo.[app_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[app_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Catalog].[Category] WHERE [Category].[ParentCategoryId] = 0
        UNION ALL
        SELECT [Category].[Id] AS [Id], [CategoryTree].[Order] + '|' + dbo.[app_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + dbo.[app_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Catalog].[Category]
        INNER JOIN [Catalog].[CategoryTree] ON [CategoryTree].[Id] = [Category].[ParentCategoryId])
    INSERT INTO #OrderedCategoryIds ([CategoryId])
    SELECT [Category].[Id]
    FROM [Catalog].[CategoryTree]
    RIGHT JOIN [Catalog].[Category] ON [CategoryTree].[Id] = [Category].[Id]
    ...
    AND (@ShowHidden = 1 OR @FilteredUserRoleIdsCount  = 0 OR [Category].[SubjectToAcl] = 0
        OR EXISTS (SELECT 1 FROM #FilteredUserRoleIds [roles] WHERE [roles].[UserRoleId] IN
            (SELECT [acl].[UserRoleId] FROM [Security].[AclRecord] acl WITH (NOLOCK) WHERE [acl].[EntityId] = [Category].[Id] AND [acl].[EntityName] = 'Category')
        )
    )
    AND (@StoreId = 0 OR [Category].[LimitedToStores] = 0
        OR EXISTS (SELECT 1 FROM Stores.[StoreMapping] sm WITH (NOLOCK)
    ...
    --paging
    SELECT [Category].* FROM #OrderedCategoryIds AS [Result] INNER JOIN [Catalog].[Category] ON [Result].[CategoryId] = [Category].[Id]
