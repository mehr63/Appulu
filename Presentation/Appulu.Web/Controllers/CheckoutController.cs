...
namespace Appulu.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual IActionResult Index()
        {
            ...
            if (_workContext.CurrentUser.IsGuest() && (!_orderSettings.AnonymousCheckoutAllowed || downloadableProductsRequireRegistration))
                return Challenge();
            ...
            var paymentMethods = _paymentService
                .LoadActivePaymentMethods(_workContext.CurrentUser, _storeContext.CurrentStore.Id)
        ...
        public virtual IActionResult SelectBillingAddress(int addressId, bool shipToSameAddress = false)
        {
            ...
            var address = _workContext.CurrentUser.Addresses.FirstOrDefault(a => a.Id == addressId);
            if (address == null)
                return RedirectToRoute("CheckoutBillingAddress");
            ...
            {
                _workContext.CurrentUser.ShippingAddress = _workContext.CurrentUser.BillingAddress;
                _userService.UpdateUser(_workContext.CurrentUser);
                //reset selected shipping method (in case if "pick up in store" was selected)
                _genericAttributeService.SaveAttribute<ShippingOption>(_workContext.CurrentUser, AppUserDefaults.SelectedShippingOptionAttribute, null, _storeContext.CurrentStore.Id);
                _genericAttributeService.SaveAttribute<PickupPoint>(_workContext.CurrentUser, AppUserDefaults.SelectedPickupPointAttribute, null, _storeContext.CurrentStore.Id);
        ...
        public virtual IActionResult NewBillingAddress(CheckoutBillingAddressModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var address = _addressService.FindAddress(_workContext.CurrentUser.Addresses.ToList(),
                ...
                if (address == null)
                {
                    ...
                    _workContext.CurrentUser.UserAddressMappings.Add(new UserAddressMapping { Address = address });
                }
                _workContext.CurrentUser.BillingAddress = address;
                _userService.UpdateUser(_workContext.CurrentUser);
        ...
        public virtual IActionResult NewShippingAddress(CheckoutShippingAddressModel model)
        {
            ...
            if (_shippingSettings.AllowPickUpInStore)
            {
                if (model.PickUpInStore)
                {
                    ...
                    var pickupPoints = _shippingService.GetPickupPoints(_workContext.CurrentUser.BillingAddress,
                    ...
                    _genericAttributeService.SaveAttribute(_workContext.CurrentUser, AppUserDefaults.SelectedShippingOptionAttribute, pickUpInStoreShippingOption, _storeContext.CurrentStore.Id);
                    _genericAttributeService.SaveAttribute(_workContext.CurrentUser, AppUserDefaults.SelectedPickupPointAttribute, selectedPoint, _storeContext.CurrentStore.Id);
        ...
        public virtual IActionResult ShippingMethod()
        {
            ...
            //model
            var model = _checkoutModelFactory.PrepareShippingMethodModel(cart, _workContext.CurrentUser.ShippingAddress);
                ...
                model.ShippingMethods.Count == 1)
            {
                ...
                _genericAttributeService.SaveAttribute(_workContext.CurrentUser,
                    AppUserDefaults.SelectedShippingOptionAttribute,
        ...
        public virtual IActionResult SelectShippingMethod(string shippingoption)
        {
            ...
            if (!_shoppingCartService.ShoppingCartRequiresShipping(cart))
            {
                _genericAttributeService.SaveAttribute<ShippingOption>(_workContext.CurrentUser,
                    AppUserDefaults.SelectedShippingOptionAttribute, null, _storeContext.CurrentStore.Id);
                return RedirectToRoute("CheckoutPaymentMethod");
            }
            ...
            var shippingOptions = _genericAttributeService.GetAttribute<List<ShippingOption>>(_workContext.CurrentUser,
                AppUserDefaults.OfferedShippingOptionsAttribute, _storeContext.CurrentStore.Id);
            if (shippingOptions == null || !shippingOptions.Any())
            {
                //not found? let's load them using shipping service
                shippingOptions = _shippingService.GetShippingOptions(cart, _workContext.CurrentUser.ShippingAddress,
            ...
            //save
            _genericAttributeService.SaveAttribute(_workContext.CurrentUser, AppUserDefaults.SelectedShippingOptionAttribute, shippingOption, _storeContext.CurrentStore.Id);
        ...
        public virtual IActionResult PaymentMethod()
        {
            ...
            if (!isPaymentWorkflowRequired)
            {
                _genericAttributeService.SaveAttribute<string>(_workContext.CurrentUser,
                    AppUserDefaults.SelectedPaymentMethodAttribute, null, _storeContext.CurrentStore.Id);
            ...
            if (_addressSettings.CountryEnabled &&
                _workContext.CurrentUser.BillingAddress != null &&
                _workContext.CurrentUser.BillingAddress.Country != null)
            {
                filterByCountryId = _workContext.CurrentUser.BillingAddress.Country.Id;
            }
                ...
                paymentMethodModel.PaymentMethods.Count == 1 && !paymentMethodModel.DisplayRewardPoints)
            {
                ...
                _genericAttributeService.SaveAttribute(_workContext.CurrentUser,
                    AppUserDefaults.SelectedPaymentMethodAttribute,
        ...
        public virtual IActionResult SelectPaymentMethod(string paymentmethod, CheckoutPaymentMethodModel model)
        {
            ...
            {
                _genericAttributeService.SaveAttribute(_workContext.CurrentUser,
                    AppUserDefaults.UseRewardPointsDuringCheckoutAttribute, model.UseRewardPoints,
                    _storeContext.CurrentStore.Id);
            }
            ...
            if (paymentMethodInst == null ||
                ...
                !_pluginFinder.AuthorizedForUser(paymentMethodInst.PluginDescriptor, _workContext.CurrentUser))
                return PaymentMethod();

            //save
            _genericAttributeService.SaveAttribute(_workContext.CurrentUser,
                AppUserDefaults.SelectedPaymentMethodAttribute, paymentmethod, _storeContext.CurrentStore.Id);
        ...
        {
            var shippingMethodModel = _checkoutModelFactory.PrepareShippingMethodModel(cart, _workContext.CurrentUser.ShippingAddress);
            if (_shippingSettings.BypassShippingMethodSelectionIfOnlyOne &&
        ...
        public virtual IActionResult OpcSaveBilling(CheckoutBillingAddressModel model)
        {
            try
            {
                ...
                {
                    //existing address
                    var address = _workContext.CurrentUser.Addresses.FirstOrDefault(a => a.Id == billingAddressId);
                    if (address == null)
                ...
                if (_shoppingCartService.ShoppingCartRequiresShipping(cart))
                {
                    ...
                    if (_shippingSettings.ShipToSameAddress && model.ShipToSameAddress && _workContext.CurrentUser.BillingAddress.Country.AllowsShipping)
                    {
                        //ship to the same address
                        _workContext.CurrentUser.ShippingAddress = _workContext.CurrentUser.BillingAddress;
                        _userService.UpdateUser(_workContext.CurrentUser);
        ...
        public virtual IActionResult OpcSaveShipping(CheckoutShippingAddressModel model)
        {
            try
            {
                ...
                {
                    //existing address
                    var address = _workContext.CurrentUser.Addresses.FirstOrDefault(a => a.Id == shippingAddressId);
                    if (address == null)
                        throw new Exception("Address can't be loaded");
