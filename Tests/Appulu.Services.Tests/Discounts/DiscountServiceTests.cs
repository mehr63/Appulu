    ...
    public class DiscountServiceTests : ServiceTest
    {
        ...
        public new void SetUp()
        {
            ...
            var discount2 = new Discount
            {
                ...
                DiscountLimitation = DiscountLimitationType.NTimesPerUser,
                LimitationTimes = 3
            };
        ...
        public void Should_accept_valid_discount_code()
        {
            ...
            //we have to register "app_cache_static" cache manager (null manager) from DependencyRegistrar.cs
            //because DiscountService right now dynamically Resolve<ICacheManager>("app_cache_static")
            //we cannot inject it because DiscountService already has "per-request" cache manager injected 
            //EngineContext.Initialize(false);

            _discountService.ValidateDiscount(discount, user, new[] { "CouponCode 1" }).IsValid.ShouldEqual(true);
        }
        ...
        public void Should_not_accept_wrong_discount_code()
        {
            ...
            _discountService.ValidateDiscount(discount, user, new[] { "CouponCode 2" }).IsValid.ShouldEqual(false);
        }
        ...
        public void Can_validate_discount_dateRange()
        {
            ...
            _discountService.ValidateDiscount(discount, user, null).IsValid.ShouldEqual(true);

            discount.StartDateUtc = DateTime.UtcNow.AddDays(1);
            _discountService.ValidateDiscount(discount, user, null).IsValid.ShouldEqual(false);
        }
